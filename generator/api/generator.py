...
HOME_PATH = Path("generator/api")
DESTINATION_PATH = Path("tgramclient/raw")
NOTICE_PATH = "NOTICE"
...
def get_docstring_arg_type(t: str):
    if t in CORE_TYPES:
        ...
        return f"``{t.lower()}``"
    if t in {"TLObject", "X"}:
        return "Any object from :obj:`~tgramclient.raw.types`"
    if t == "!X":
        return "Any function from :obj:`~tgramclient.raw.functions`"
    if t.lower().startswith("vector"):
        return "List of " + get_docstring_arg_type(t.split("<", 1)[1][:-1])
    return f":obj:`{t} <tgramclient.raw.base.{t}>`"
...
def start(format: bool = False):
    ...
    for c in combinators:
        ...
        if c.section == "types":
            ...
            docstring += f"\n    Constructor of :obj:`~tgramclient.raw.base.{c.qualtype}`."
    ...
    with open(DESTINATION_PATH / "all.py", "w", encoding="utf-8") as f:
        ...
        for c in combinators:
            f.write(f'\n    {c.id}: "tgramclient.raw.{c.section}.{c.qualname}",')

        f.write('\n    0xbc799737: "tgramclient.raw.core.BoolFalse",')
        f.write('\n    0x997275b5: "tgramclient.raw.core.BoolTrue",')
        f.write('\n    0x1cb5c415: "tgramclient.raw.core.Vector",')
        f.write('\n    0x73f1f8dc: "tgramclient.raw.core.MsgContainer",')
        f.write('\n    0xae500895: "tgramclient.raw.core.FutureSalts",')
        f.write('\n    0x0949d9dc: "tgramclient.raw.core.FutureSalt",')
        f.write('\n    0x3072cfa1: "tgramclient.raw.core.GzipPacked",')
        f.write('\n    0x5bb8e511: "tgramclient.raw.core.Message",')
